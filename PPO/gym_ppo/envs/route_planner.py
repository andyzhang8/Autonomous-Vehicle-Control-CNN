...
from gym_ppo.envs.misc import distance_vehicle, is_within_distance_ahead, compute_magnitude_angle
...
class RoutePlanner():
  def __init__(self, vehicle, buffer_size, destination=None):
    ...
    maxlen=10
    if destination:
      maxlen = math.sqrt((destination.location.x - self._current_waypoint.transform.location.x) ** 2 + 
                (destination.location.x - self._current_waypoint.transform.location.x) ** 2) / self._sampling_radius

    self._waypoints_queue = deque(maxlen=int(maxlen * 0.8))
    ...
    self._compute_next_waypoints(k=600)
    self.destination = self._waypoints_queue[-1]
  ...
  def _compute_next_waypoints(self, k=1):
    ...
    for _ in range(k):
      ...
      for waypoint in self._waypoints_queue:
        color = carla.Color(r=0,g=165,b=30)
        self._world.debug.draw_string(waypoint[0].transform.location, 'X', 
                draw_shadow=False, color=color, life_time=5.0)
  ...
  def _get_waypoints(self):
    ...
    # self._world.debug.draw_string(self._target_waypoint.transform.location, 'Y', 
    #             draw_shadow=False, color=carla.Color(r=255, g=0, b=0), life_time=1.0)
    ...
    if len(self._waypoints_queue) < int(1):
      self.route_done = True
    return waypoints

  def get_wayp_vec(self):
    dis, angle = (0,0)
    if self._vehicle != None and self._target_waypoint != None:
      dis = self._vehicle.get_transform().location.distance(self._target_waypoint.transform.location)
      angle = (self._vehicle.get_transform().rotation.yaw - self._target_waypoint.transform.rotation.yaw) % 360
